...
While we have introduced the Camunda Exporter to simplify things, and get several benefits from it.

For the migration/update phase we still need to run Operate and Tasklist importer and the Elasticsearch Exporter. This is to make sure that old
...
We were able to reduce the need of several indices and to make use of existing ones (Operate indices are used by Tasklist, etc.)

### Notes for updating to 8.7

Notes for updating to 8.7 related to the Camunda Exporter and harmonized indices.

#### Users need to make sure that exporter/import backlog is small
...
> [!Important]
>
> To reduce the here potential of building a backlog, which can cause on the Zeebe side also issues
...
As we have harmonized indices there are some details Tasklist needs as part of the processes, which are not part of
existing Operate indices.
...
> [!Note:]
>
> If you run the single application, it should be run automatically (built-in)
...
#### Custom prefix usage need extra care
...
1. Shutdown your Camunda 8.7 cluster, fex. reduce the replicas to zero. Such that no traffic will executed against your to be migrated ES/OS prefixes.
2. In the `camunda/bin` directory there is a `prefix-migration` script that will execute the prefix migration,
   ...
   2. Alternative, you can set them via env vars.
